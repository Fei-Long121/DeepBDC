...
def filter_parameters(parameters, exclude_param):
    return [p for p in parameters if id(p) != id(exclude_param)]
...
def train(params, base_loader, val_loader, model, stop_epoch):
    ...
    if params.method in ['stl_deepbdc', 'meta_deepbdc']:
        ...
        bas_params = filter_parameters(model.parameters(), model.dcov.temperature)
        optimizer = torch.optim.SGD([
            {'params': bas_params},
            {'params': model.dcov.temperature, 'lr': params.t_lr}], lr=params.lr, weight_decay=params.wd, nesterov=True,
            momentum=0.9)
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=params.lr, weight_decay=params.wd, nesterov=True,
                                    momentum=0.9)
        ...
        if params.dataset == 'tiered_imagenet':
            if epoch >= params.milestones[1]:
                ...
                if params.val in ['meta']:
                    ...
                    print("model best acc is {:.2f}, best acc epoch is {}".format(trlog['max_acc'],
                                                                                  trlog['max_acc_epoch']))
        else:
            print("开始eval")
            model.eval()
